  ...
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-1.5">
      ...
      >
        ...
        <div className="flex min-w-0 items-center justify-between gap-2 font-mono text-[11px] uppercase text-text-muted"><span className="min-w-0 truncate">{finding.assetId ? nodeById.get(finding.assetId)?.label ?? finding.assetId : 'Project'}</span><span className="shrink-0">{finding.confidence} / {finding.status}</span></div>
      </button>)}
    </div>
  </div>;
}
