...
export function FindingsTab({ showHeader = true, onRecordContextMenu }: {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    {showHeader && <div className="flex shrink-0 items-center justify-between border-b border-border-subtle bg-panel/50 px-3 py-2">
      <div><div className="text-xs font-medium text-text-secondary">Findings</div><div className="text-[11px] text-text-muted">Reusable project knowledge</div></div>
      <button aria-label="Create finding" onClick={() => setShowCreate((value) => !value)} className="ui-icon-button h-7 w-7 border-border-subtle text-accent-blue hover:bg-accent-blue/10"><Icon name="plus" size={13} /></button>
    </div>}
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-1.5">
      ...
      >
        <div className="mb-1 flex min-w-0 items-start justify-between gap-2"><span className="min-w-0 flex-1 truncate text-2xs font-medium leading-relaxed text-text-primary">{finding.title}</span><span className="shrink-0 font-mono text-[11px] uppercase text-accent-teal">{finding.kind}</span></div>
        <div className="flex min-w-0 items-center justify-between gap-2 font-mono text-[11px] uppercase text-text-muted"><span data-presentation-sensitive className="min-w-0 truncate">{finding.assetId ? nodeById.get(finding.assetId)?.label ?? finding.assetId : 'Project'}</span><span className="shrink-0">{finding.confidence} / {finding.status}</span></div>
      </button>)}
    </div>
  </div>;
}
...
function CreateFinding({ initialAssetId, assets, onCreate }: {
  ...
  return <div className="shrink-0 space-y-2 border-b border-border-subtle bg-panel/70 p-3">
    ...
    <div className="flex gap-2"><select aria-label="Finding kind" value={kind} onChange={(event) => setKind(event.target.value as FindingKind)} className="h-7 min-w-0 flex-1 rounded border border-border-subtle bg-panel px-2 text-2xs text-text-primary">{KINDS.map((value) => <option key={value}>{value}</option>)}</select><select aria-label="Finding confidence" value={confidence} onChange={(event) => setConfidence(event.target.value as FindingConfidence)} className="h-7 min-w-0 flex-1 rounded border border-border-subtle bg-panel px-2 text-2xs text-text-primary">{CONFIDENCES.map((value) => <option key={value}>{value}</option>)}</select><button disabled={!title.trim()} onClick={() => void onCreate({ assetId: assetId || undefined, title: title.trim(), kind, confidence })} className="rounded border border-accent-blue/40 px-3 text-2xs text-accent-blue disabled:opacity-30">Add</button></div>
  </div>;
}
