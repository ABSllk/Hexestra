...
function RecordList({ rows, empty, onSelect, onRecordContextMenu, sensitiveMeta = false }: { rows: Array<{ id: string; title: string; meta: string; badge: string }>; empty: string; onSelect: (id: string) => void; onRecordContextMenu: (event: React.MouseEvent<HTMLButtonElement>, recordId: string) => void; sensitiveMeta?: boolean }) {
  return <div className="h-full overflow-y-auto p-1.5">{rows.length === 0 ? <div className="p-4 text-center text-2xs text-text-muted">{empty}</div> : rows.map((row) => <button key={row.id} onClick={() => onSelect(row.id)} onContextMenu={(event) => onRecordContextMenu(event, row.id)} className="ui-hover-row mb-1 w-full min-w-0 px-2.5 py-2 text-left"><div className="mb-1 flex min-w-0 items-start justify-between gap-2"><span className="min-w-0 flex-1 truncate text-2xs font-medium leading-relaxed text-text-primary">{row.title}</span><span className="max-w-[45%] shrink-0 truncate rounded-md bg-accent-teal/5 px-1 font-mono text-[11px] uppercase text-accent-teal" title={row.badge}>{row.badge}</span></div><div data-presentation-sensitive={sensitiveMeta ? '' : undefined} className="truncate font-mono text-[11px] text-text-muted">{row.meta}</div></button>)}</div>;
}
