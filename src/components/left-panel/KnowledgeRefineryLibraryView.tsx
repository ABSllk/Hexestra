...
export function KnowledgeRefineryLibraryView() {
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-canvas">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-1.5">
      ...
      {!loading && view === 'sources' && filteredSources.map((source) => <article key={source.id} className={cn('mb-1 rounded border p-2', selectedSourceId === source.id ? 'border-accent-blue/45 bg-accent-blue/8' : 'border-border-subtle bg-panel/40 hover:border-border-strong')}>
        <button type="button" onClick={() => { selectSource(source.id); openKnowledgeRefineryTab(undefined, source.id); }} className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"><div className="flex min-w-0 items-start gap-2"><Icon name="file" size={13} className="mt-0.5 text-accent-teal" /><div className="min-w-0 flex-1"><p className="truncate text-[11px] font-medium text-text-primary">{source.name}</p><p className="mt-0.5 text-[10px] text-text-muted">{source.format?.toUpperCase() ?? 'TEXT'} · {source.sourceAvailable ? t('refinery.retained') : t('refinery.missing')}</p></div></div></button>
        <div className="mt-2 flex justify-end gap-1"><button type="button" disabled={!source.sourceAvailable || isAgentProcessing || startingSourceId !== null} onClick={() => void distillInNewConversation(source.id)} className="inline-flex h-6 items-center gap-1 rounded border border-accent-blue/30 px-1.5 text-[10px] text-accent-blue hover:bg-accent-blue/10 disabled:opacity-40"><Icon name="play" size={10} />{t('refinery.refine')}</button><button type="button" onClick={() => setConfirming({ kind: 'source', id: source.id })} className="ui-icon-button h-6 w-6" aria-label={`${t('refinery.deleteSource')}: ${source.name}`} title={t('refinery.deleteSource')}><Icon name="trash" size={11} /></button></div>
      ...
      {!loading && view === 'jobs' && filteredJobs.map((job) => <article key={job.id} className={cn('mb-1 rounded border p-2', selectedJobId === job.id ? 'border-accent-blue/45 bg-accent-blue/8' : 'border-border-subtle bg-panel/40 hover:border-border-strong')}>
        <button type="button" onClick={() => openJob(job.id)} className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"><div className="flex items-start gap-2"><Icon name={job.status === 'failed' ? 'alert' : job.status === 'review_required' ? 'check' : 'sparkles'} size={13} className={cn('mt-0.5', job.status === 'failed' ? 'text-severity-critical' : job.status === 'review_required' ? 'text-accent-teal' : 'text-accent-blue')} /><div className="min-w-0 flex-1"><p className="truncate text-[11px] font-medium text-text-primary">{job.source.name}</p><p className="mt-0.5 text-[10px] text-text-muted">{t(jobStatusKey(job.status))} · {job.candidateCounts.restriction + job.candidateCounts.skill + job.candidateCounts.workflow} {t('refinery.candidates')}</p></div></div></button>
        <div className="mt-2 flex justify-end"><button type="button" onClick={() => setConfirming({ kind: 'job', id: job.id })} className="ui-icon-button h-6 w-6" aria-label={`${t('refinery.deleteRun')}: ${job.source.name}`} title={t('refinery.deleteRun')}><Icon name="trash" size={11} /></button></div>
