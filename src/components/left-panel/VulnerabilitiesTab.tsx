...
export function VulnerabilitiesTab({ onRecordContextMenu }: {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    <div className="min-h-0 flex-1 overflow-y-auto p-1.5">{records.length === 0 ? <div className="p-4 text-center text-2xs text-text-muted">No validated vulnerabilities.</div> : records.map((record) => <button key={record.id} onClick={() => { selectNode(record.assetId); openRecordTab('vulnerability', record.id, record.title); }} onContextMenu={(event) => onRecordContextMenu?.(event, record.id)} className="ui-hover-row mb-1 w-full px-2.5 py-2 text-left"><div className="mb-1 flex justify-between gap-2"><span className="text-2xs font-medium text-text-primary">{record.title}</span><Severity value={record.severity} /></div><div className="flex justify-between font-mono text-[8px] uppercase text-text-muted"><span className="truncate">{labels.get(record.assetId) ?? record.assetId}</span><span>{record.status}</span></div></button>)}</div>
  </div>;
}
