...
function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) {
  return <select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="ui-control h-7 min-w-0 px-1 text-[9px] uppercase text-text-secondary">{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>;
}

function EmptyAssets({ hasAny }: { hasAny: boolean }) {
  const { t } = useI18n();
  return <div className="flex h-full flex-col items-center justify-center p-4 text-text-muted"><div className="mb-3 flex h-11 w-11 items-center justify-center rounded-full border border-surface bg-bg-tertiary"><Icon name="target" size={22} /></div><p className="mb-1 text-center text-xs font-medium text-text-secondary">{hasAny ? t('assets.noMatching') : t('assets.none')}</p><p className="text-center text-2xs">{hasAny ? t('assets.changeFilters') : t('assets.emptyHint')}</p></div>;
}
...
function ChangesPanel() {
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto">
    ...
    {changes.length === 0 ? <div className="p-4 text-center text-2xs text-text-muted">No material changes observed yet.</div> : changes.map((change) => <div key={change.id} className="border-b border-surface/50 px-3 py-2">
      ...
      {(change.before || change.after) && <div className="mt-1 truncate font-mono text-[9px] text-text-muted">{change.before ? `${change.before} → ` : ''}{change.after}</div>}
      <div className="mt-1 font-mono text-[8px] text-text-muted/70">{formatTime(change.observedAt)}</div>
    </div>)}
  </div>;
}
