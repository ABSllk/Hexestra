...
function WorkflowRow({ workflow, confirming, onOpen, onRun, onCopy, onExport, onDelete, t }: { workflow: WorkflowSummary; confirming: boolean; onOpen: (mode: 'preview' | 'edit') => void; onRun: () => void; onCopy: () => void; onExport: () => void; onDelete: () => void; t: (key: TranslationKey, values?: Record<string, string | number>) => string }) {
  return <article className={cn('mb-1 rounded border bg-panel/45 p-2.5 transition-colors', workflow.valid ? 'border-border-subtle hover:border-border-strong' : 'border-severity-medium/35 bg-severity-medium/5')}>
    <button type="button" onClick={onRun} className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"><div className="flex items-start gap-2"><Icon name="sparkles" size={13} className={workflow.valid ? 'mt-0.5 text-accent-blue' : 'mt-0.5 text-severity-medium'} /><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><span className="truncate text-xs font-medium text-text-primary">{workflow.name}</span><span className="shrink-0 font-mono text-[9px] text-text-muted">{workflow.version || t('workflow.invalid')}</span></div><p className="mt-1 line-clamp-2 text-[10px] leading-4 text-text-muted">{workflow.description || workflow.diagnostics[0] || t('workflow.objectiveHint')}</p></div></div></button>
    <div className="mt-2 flex items-center justify-between gap-1.5"><div className="flex min-w-0 flex-wrap gap-1">{workflow.tags.slice(0, 3).map((tag) => <span key={tag} className="rounded bg-accent-blue/8 px-1.5 py-0.5 font-mono text-[9px] text-accent-blue">{tag}</span>)}</div><div className="flex shrink-0 items-center gap-0.5"><button type="button" onClick={onRun} className="rounded border border-accent-blue/30 px-1.5 py-1 text-[10px] text-accent-blue hover:bg-accent-blue/10" title={t('workflow.run')}><Icon name="send" size={11} /></button><button type="button" onClick={() => onOpen('edit')} className="ui-icon-button h-6 w-6" title={t('workflow.edit')} aria-label={t('workflow.edit')}><Icon name="edit" size={11} /></button><button type="button" onClick={onCopy} className="ui-icon-button h-6 w-6" title={t('workflow.copy')} aria-label={t('workflow.copy')}><Icon name="copy" size={11} /></button><button type="button" onClick={onExport} className="ui-icon-button h-6 w-6" title={t('workflow.export')} aria-label={t('workflow.export')}><Icon name="file" size={11} /></button><button type="button" onClick={onDelete} className="ui-icon-button h-6 w-6 text-severity-medium" title={t('workflow.delete')} aria-label={t('workflow.delete')}><Icon name={confirming ? 'check' : 'trash'} size={11} /></button></div></div>
