      ...
      <div className="mx-auto max-w-3xl px-8 py-7">
        ...
        <section className="mb-6 grid grid-cols-[180px_1fr] gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded border border-border-subtle bg-panel/45 p-3 text-[11px] leading-4 text-text-muted">
...
function Field({ label, hint, children }: { label: string; hint: string; children: ReactNode }) {
  return <label className="block"><span className="mb-1 flex items-baseline justify-between gap-3"><span className="text-xs font-medium text-text-secondary">{label}</span><span className="text-[11px] text-text-muted">{hint}</span></span>{children}</label>;
}
