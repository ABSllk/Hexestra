...
function RecordDetail({ kind, recordId }: { kind: ManagedRecordKind; recordId: string }) {
  ...
  if (kind === 'evidence') {
    ...
    return record ? <Page title={record.title} eyebrow="Evidence" subtitle={`${labels.get(record.assetId) ?? record.assetId} / ${record.tool} / ${record.kind}`}>
      <Section title="Raw tool output"><pre className="max-h-[65vh] overflow-auto whitespace-pre-wrap break-words rounded border border-border-subtle bg-panel p-4 font-mono text-[11px] leading-5 text-text-secondary">{record.content || 'No raw output.'}</pre></Section>
  ...
  return record ? <Page title={record.title} eyebrow="Report" subtitle={`${record.status} / ${formatTime(record.updatedAt)}`}>
    ...
    <Section title="Content"><div className="rounded border border-border-subtle bg-panel/50 p-4 text-sm leading-6 text-text-secondary"><MarkdownContent content={record.content || '_Empty report_'} /></div></Section>
    <References findingIds={record.findingIds} vulnerabilityIds={record.vulnerabilityIds} />
  </Page> : <MissingRecord />;
}
...
function Page({ title, eyebrow, subtitle, children }: { title: string; eyebrow: string; subtitle: string; children: React.ReactNode }) {
  return <div className="h-full overflow-y-auto bg-canvas px-8 py-7">
    <div className="mx-auto max-w-5xl space-y-5">
      <header className="border-b border-border-subtle pb-5"><div className="mb-2 font-mono text-[11px] uppercase tracking-[0.18em] text-accent-teal">{eyebrow}</div><h1 className="text-xl font-semibold text-text-primary">{title}</h1><div className="mt-2 font-mono text-[11px] text-text-muted">{subtitle}</div></header>
      {children}
    </div>
  </div>;
}
...
function References({ findingIds = [], vulnerabilityIds = [], evidenceIds = [] }: { findingIds?: string[]; vulnerabilityIds?: string[]; evidenceIds?: string[] }) {
  ...
  return <div className="space-y-2">{groups.filter(({ ids }) => ids.length > 0).map(({ label, kind, ids, titles }) => <div key={label}>
    <div className="mb-1 text-[11px] uppercase tracking-wide text-text-muted">Linked {label}</div>
    <div className="flex flex-wrap gap-1">{ids.map((id) => {
      const title = titles.get(id);
      return title
        ? <button key={id} onClick={() => openRecordTab(kind, id, title)} className="rounded border border-border-subtle bg-panel px-2 py-1 text-left text-[11px] text-accent-blue hover:border-accent-blue/40 hover:bg-accent-blue/10">{title}</button>
        : <span key={id} className="rounded border border-border-subtle bg-panel px-2 py-1 text-[11px] text-text-muted">Unavailable record</span>;
    })}</div>
  </div>)}</div>;
}
