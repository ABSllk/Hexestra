...
> 维护者参考。普通用户请先阅读[使用指南](user-guide.zh-CN.md)。
...
核心记录之间的关系见[领域模型](domain-model.zh-CN.md)，Agent 的执行细节见
[Agent 运行机制](agent-runtime.zh-CN.md)。
...
- [领域模型](domain-model.zh-CN.md)：Project、Scope、Asset、Task 与安全记录如何关联。
- [Agent 运行机制](agent-runtime.zh-CN.md)：上下文、审批、队列、事件、持久化与分支语义。
