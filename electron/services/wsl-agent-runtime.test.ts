...
describe('WSL Agent runtime', () => {
  ...
  it('checks the ANTHROPIC_BASE_URL from WSL Claude user settings', async () => {
  ...
  it('reports an invalid configured provider URL without probing the default endpoint', async () => {
    const calls: Array<{ command: string; args: string[] }> = [];
    const runFile = vi.fn(async (command: string, args: string[]) => {
      calls.push({ command, args });
      if (args.includes('--version')) return { stdout: '2.1.0', stderr: '' };
