...
vi.mock('./terminal.service', () => ({ terminatePtyProcessTree: vi.fn() }));
vi.mock('./shell-vault', () => ({
  shellVault: {
    list: vi.fn(() => []),
    save: vi.fn(),
...
import { LOCAL_OPERATOR_ASSET_ID } from '../contracts/shell';
...
describe('ShellService local session and Agent lease', () => {
  ...
  it('starts a selected WSL distribution through node-pty without shell interpolation', async () => {
    const profile = service.saveProfile('project-1', {
